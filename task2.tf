provider "aws" {
  ...
  region   = "ap-south-1"
  profile  = "default"
  access_key = "AKIAJS5GNZI7D4IYLUPA"
  secret_key = "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"
}
 ...
 depends_on = [ 
  ...
  provisioner "remote-exec" {
      inline = [
        ...
        "sudo curl https://github.com/PatelAayush/task2hybridmulticloud.git > index.html",                                  
...
resource "aws_s3_bucket_object" "tk2-object" {
  ...
  key    = "one"
  source = "/home/aay/Documents/LW tasks/HMC/task2/image.jpeg"
  etag = "/home/aay/Documents/LW tasks/HMC/task2/image.jpeg"
  acl = "public-read"
...
resource "aws_cloudfront_distribution" "tk2-s3_distribution" {
  ...
  default_root_object = "image.jpeg"
...
resource "null_resource" "nulllocal3" {
  depends_on = [
   ...
   provisioner "local-exec" {
         command = "google-chrome ${aws_instance.tk2_os.public_ip}/index.html"
    }
}
